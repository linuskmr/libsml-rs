...
/// See <https://github.com/volkszaehler/libsml/blob/master/sml/include/sml/sml_get_list_response.h>
pub struct GetListResponse<'a>(pub(crate) *mut libsml_sys::sml_get_list_response, PhantomData<&'a ()>);

impl<'a> GetListResponse<'a> {
	pub fn new() -> Self {
		GetListResponse(unsafe { libsml_sys::sml_get_list_response_init() }, PhantomData)
	}
	...
	pub fn set_act_gateway_time(&mut self, act_gateway_time: Option<&'a Time>) {
		unsafe { (*self.0).act_gateway_time = act_gateway_time.map_or(std::ptr::null_mut(), |v| v.0) }
	}
}